...
export interface WaccPais {
  codigo: string;
  nome: string;
  crp: number;
}

/** Executa o cálculo de WACC */
export async function runWaccCalculation(request: WaccRequest): Promise<WaccResult> {
  const { data } = await api.post<WaccResult>('/wacc/calcular', request);
  return data;
}

/** Busca setores com betas Damodaran */
export async function getWaccSetores(): Promise<WaccSetor[]> {
  const { data } = await api.get<WaccSetor[]>('/wacc/setores');
  return data;
}

/** Busca países com CRP */
export async function getWaccPaises(): Promise<WaccPais[]> {
  const { data } = await api.get<WaccPais[]>('/wacc/paises');
  return data;
}

/** Busca parâmetros de mercado sugeridos */
export async function getWaccMarketParams(): Promise<{ rf: number; erp: number; fonte: string }> {
  const { data } = await api.get('/wacc/parametros-mercado');
  return data;
}
