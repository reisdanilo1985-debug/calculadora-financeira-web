 ...
 */
export function parseEttjAnbima(csv: string): CurvasTesouro {
  ...
  const real: VerticeCurva[] = [];
  const idxIpca = linhas.findIndex(l => /^vertices;ettj ipca/i.test(l));
  if (idxIpca >= 0) {
    for (let i = idxIpca + 1; i < linhas.length; i++) {
      const l = linhas[i];
      if (!l) break; // tabela termina na linha em branco
      const c = l.split(';');
      const prazo = numBr(c[0] ?? '');
      const taxa = numBr(c[1] ?? '');
      if (!isFinite(prazo) || !isFinite(taxa)) break;
