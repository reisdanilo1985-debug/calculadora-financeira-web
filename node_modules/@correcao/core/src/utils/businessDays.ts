...
function getMobileHolidays(year: number): string[] {
  ...
  const addOffset = (base: Date, offset: number): string => {
 ...
 */
export function getCalendarDaysBetween(startDate: Date, endDate: Date): Date[] {
  const days: Date[] = [];
  let current = new Date(startDate);
  current.setHours(0, 0, 0, 0);
  const end = new Date(endDate);
 ...
 */
export function parseBCBDate(str: string): Date {
 ...
 */
export function lastBusinessDayOn(date: Date): Date {
  let d = new Date(date);
  d.setHours(0, 0, 0, 0);
